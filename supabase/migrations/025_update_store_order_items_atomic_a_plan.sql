-- ============================================================
-- 025_update_store_order_items_atomic_a_plan.sql
-- A안 적용 — 가맹점 발주 수정 RPC 를 reserved 도 같이 움직이도록 갱신.
-- ============================================================
-- 배경:
--   017 에서 정의된 update_store_order_items_atomic 함수는
--   기존 코드 동작(발주 시점에 inventory.quantity 만 차감)을 그대로 따르고 있다.
--   023 에서 reserved/on_hand 컬럼을 추가했으므로,
--   수정 시 박스 path 가 reserved 도 같이 조정해야 새 컬럼이 정확히 유지된다.
--
-- 변경 핵심 (박스 path 만 손봄, 팩 path 는 apply_b2b_inventory_delta 그대로 사용):
--   v_qty_diff 양수(수량 증가) → quantity -= diff, reserved += diff
--   v_qty_diff 음수(수량 감소) → quantity += |diff|, reserved -= |diff|
--   on_hand 는 안 건드림 (예약 변동일 뿐 신화 창고 변동 아님).
--
-- 멱등성: CREATE OR REPLACE FUNCTION 이므로 재실행 안전.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    IF v_unit = 'box' THEN
      ...
      -- A안: reserved 도 같이 조정. quantity 와 reserved 를 같이 fetch.
    ...
    ELSE
      -- 팩 path: apply_b2b_inventory_delta 그대로. reserved_pack 정합성은 PR2 후속에서 다룸.
